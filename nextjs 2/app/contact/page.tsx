    ...
    <div>
      <section className="bg-gradient-hero py-16 md:py-24">
        <div className="container text-center max-w-2xl">
          ...
          <p className="text-foreground/80 mt-5 text-lg">Call, email, or send us a message — a real local technician answers, and we respond fast because plumbing problems don't wait.</p>
          <p className="text-foreground/75 mt-4 text-base">Most non-emergency calls are scheduled same-day or next-day. Emergencies get priority dispatch around the clock. Quotes are always free, always in writing, always before any work begins.</p>
        </div>
      ...
      <section className="container py-16 md:py-20 grid lg:grid-cols-[1.2fr_1fr] gap-10">
        ...
        <aside className="space-y-5">
          ...
          <ul className="stamp-card p-6 space-y-4 text-sm">
            {[{ icon: Phone, label: "Phone", val: "289-488-1007", href: "tel:+12894881007" }, { icon: Mail, label: "Email", val: EMAIL, href: `mailto:${EMAIL}` }].map(({ icon: Icon, label, val, href }) => (
            ...
            <li className="flex items-start gap-3"><div className="h-10 w-10 rounded-xl bg-primary text-primary-foreground grid place-items-center shrink-0"><MapPin className="h-5 w-5" /></div><div><div className="font-display text-primary">Office</div><a href={GOOGLE_MAPS_URL} target="_blank" rel="noopener noreferrer" className="text-foreground/80 hover:underline block">{ADDRESS_LINE}</a><div className="text-foreground/60 text-xs mt-1">Serving every city in the Niagara Region</div></div></li>
            <li className="flex items-start gap-3"><div className="h-10 w-10 rounded-xl bg-primary text-primary-foreground grid place-items-center shrink-0"><Clock className="h-5 w-5" /></div><div><div className="font-display text-primary">Hours</div><div className="text-foreground/80"><div className="font-semibold text-foreground">24 / 7 Emergency Service</div>Regular hours: Mon–Fri, 8 AM – 4 PM<div className="text-foreground/60 text-xs mt-1">After-hours rates apply on evenings &amp; weekends</div></div></div></li>
          </ul>
        </aside>
      </section>
      <section className="container pb-16 md:pb-20">
        <div className="text-center max-w-2xl mx-auto mb-8"><p className="font-script text-2xl text-accent">Find us</p><h2 className="font-display text-3xl md:text-4xl text-primary mt-1">Visit our Welland office</h2><p className="text-foreground/75 mt-3"><a href={GOOGLE_MAPS_URL} target="_blank" rel="noopener noreferrer" className="hover:underline font-semibold">{ADDRESS_LINE}</a></p></div>
        <div className="rounded-[2rem] overflow-hidden border-4 border-foreground/10 shadow-pop">
          <iframe title="Map showing Ottr Plumr at 187 King St, Welland, ON L3B 3J4" src="https://www.google.com/maps?q=187+King+St,+Welland,+ON+L3B+3J4&output=embed" width="100%" height="450" loading="lazy" referrerPolicy="no-referrer-when-downgrade" className="w-full h-[360px] md:h-[450px] block" allowFullScreen />
        </div>
      </section>
    </div>
  );
