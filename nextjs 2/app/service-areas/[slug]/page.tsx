...
export default function ServiceAreaPage({ params }: { params: { slug: string } }) {
  ...
  const ld = [
    { "@context": "https://schema.org", "@type": "Service", name: `Plumbing & Heating in ${city.name}`, description: `Local plumbing and heating service in ${city.name}, Ontario — same-day, licensed, warrantied. 24/7 emergency response.`, provider: { "@id": "https://plumr.ca/#business" }, areaServed: { "@type": "City", name: city.name, containedInPlace: "Niagara Region, Ontario, Canada" }, url: `https://plumr.ca/service-areas/${city.slug}` },
    { "@context": "https://schema.org", "@type": "BreadcrumbList", itemListElement: [{ "@type": "ListItem", position: 1, name: "Home", item: "https://plumr.ca/" }, { "@type": "ListItem", position: 2, name: "Service Areas", item: "https://plumr.ca/service-areas" }, { "@type": "ListItem", position: 3, name: city.name, item: `https://plumr.ca/service-areas/${city.slug}` }] },
  ];
  return (
    <div>
